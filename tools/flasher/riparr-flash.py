...
def sha512_crypt(pw):
    """macOS crypt(3) has no $6$ support; use passlib if present, else openssl on the Pi."""
    try:
...
def build_toml(cfg):
    psk = hashlib.pbkdf2_hmac("sha1", cfg["wifi_pw"].encode(), cfg["ssid"].encode(),
                              4096, 32).hex() if cfg["secure"] else ""
    kp = os.path.join(ASSETS, "riparr_key.pub")
    key = open(kp).read().strip() if os.path.exists(kp) else None
    esc = lambda s: s.replace("\\", "\\\\").replace('"', '\\"')
    wlan = [f'ssid = "{esc(cfg["ssid"])}"']
    if cfg["secure"]:
        wlan += [f'password = "{psk}"', "password_encrypted = true"]
    wlan += [f'hidden = {str(cfg["hidden"]).lower()}', f'country = "{esc(cfg["country"])}"']
    ssh = ["enabled = true", "password_authentication = true"]
    if key: ssh.append(f'authorized_keys = [ "{esc(key)}" ]')
    return f"""# Generated by Riparr Flasher
# Schema verified against raspberrypi-sys-mods/init_config
config_version = 1

[system]
hostname = "{esc(cfg['hostname'])}"

[user]
name = "{esc(cfg['user'])}"
password = "{cfg['pw_hash']}"
...
[locale]
keymap = "{esc(cfg['keymap'])}"
timezone = "{esc(cfg['timezone'])}"
"""
